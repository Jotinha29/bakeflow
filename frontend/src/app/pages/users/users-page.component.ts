...
@Component({
  standalone: true,
  imports: [FormsModule, ButtonModule, DialogModule, MultiSelectModule, SelectModule,
    TableModule, TagModule, TranslatePipe],
...
export class UsersPageComponent {
  ...
  readonly statusOptions = [
    { label: 'common.active', value: true }, { label: 'common.inactive', value: false },
  ];
  editing?: AuthUser;
  name = '';
  email = '';
  ...
  load(page = 0) {
    ...
    this.auth.users({ search: this.search, active: this.status ?? '', role: this.role ?? '', page, size: 20 })
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
        next: (value) => { this.users.set(value.content); this.total.set(value.total); },
        error: () => this.feedback('error', 'users.loadError'),
      });
  }

  clearFilters() { this.search = ''; this.status = null; this.role = null; this.load(); }
  ...
  save() {
    if (this.saving() || !this.name.trim() || !this.email.trim() || this.roles.length === 0 ||
      (!this.editing && this.password.length < 8)) return;
  ...
  private persist() {
    ...
    operation.pipe(finalize(() => this.saving.set(false))).subscribe({
      next: () => {
        ...
        this.feedback('success', this.editing?.active && !this.active ?
          'users.deactivated' : this.editing ? 'users.updated' : 'users.created');
        this.load();
      },
